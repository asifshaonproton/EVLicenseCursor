...
class Acr122uReader(private val context: Context) {
    ...
    companion object {
        ...
        private const val ENDPOINT_IN = 0x82 // This is an Int, but USB constants are Ints.
        private const val ENDPOINT_OUT = 0x02 // This is an Int, but USB constants are Ints.

        // ACR122U specific commands
        // Corrected: Added .toByte() to each integer literal
        private val GET_FIRMWARE_VERSION = byteArrayOf(0xFF.toByte(), 0x00.toByte(), 0x48.toByte(), 0x00.toByte(), 0x00.toByte())
        private val LOAD_AUTHENTICATION_KEYS = byteArrayOf(0xFF.toByte(), 0x82.toByte(), 0x00.toByte(), 0x00.toByte(), 0x06.toByte())
        private val GET_UID = byteArrayOf(0xFF.toByte(), 0xCA.toByte(), 0x00.toByte(), 0x00.toByte(), 0x00.toByte())

        // PICC commands for card detection
        // Corrected: Added .toByte() to each integer literal
        private val PICC_REQUEST_TYPE_A = byteArrayOf(0xFF.toByte(), 0x00.toByte(), 0x00.toByte(), 0x00.toByte(), 0x04.toByte(), 0xD4.toByte(), 0x4A.toByte(), 0x01.toByte(), 0x00.toByte())
    }
    ...
    private fun connectToDevice(device: UsbDevice): Boolean {
        ...
        for (i in 0 until interfaceDevice!!.endpointCount) {
            ...
            // UsbConstants.USB_DIR_IN and endpoint.address are Ints, so comparison is fine
